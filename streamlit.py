...
if rad == "Symptom-Based Disease Guide":
    ...
    selected_symptoms = pd.DataFrame([options], columns=X.columns)  # 선택한 증상들을 행으로 가지는 데이터프레임 생성
    selected_symptoms_encoded = pd.get_dummies(selected_symptoms)  # 더미 변수로 변환
    
    predicted_probabilities = rf_classifier.predict_proba(selected_symptoms_encoded)
